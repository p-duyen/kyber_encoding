\begin{tikzpicture}[thick]
  ...
  \begin{scope}[xshift=3.8cm]
    ...
    \draw[next] (Pt1) +(0,\msg) node[above] {$M_{t\!-\!1}$ \hspace*{.15cm}} -- (Pt1);
    \draw[next] (Pt1) -- +(.8,0);
  ...
  \begin{scope}[xshift=6.0cm]
    \draw (.5,\rate) \xor{Pt};
    \draw[next] (0,\rate) -- (Pt);
    \draw[next] (Pt) +(0,\msg) node[above] {$M_t$} -- (Pt);
    ...
    \draw[next] (Kt) -- (T);
  \end{scope}
    \draw (0.9,\rate+\phase) node {KGF};
    \draw (4.4,\rate+\phase) node {Message processing};
    \draw (8.4,\rate+\phase) node {TGF};

    \draw[dashdotted] (2.2,1.7) -- (2.2,-1.7);
    \draw[dashdotted] (7.2,1.7) -- (7.2,-1.7);
\end{tikzpicture}